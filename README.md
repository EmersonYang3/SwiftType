<div align="center">

<img src="Banner.png" alt="SwiftType Banner" width="100%" style="max-width: 800px"/>

[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg?cacheSeconds=2592000)](https://github.com/EmersonYang3/SwiftType)
[![Vite](https://img.shields.io/badge/vite-%23646CFF.svg?style=flat&logo=vite&logoColor=white)](https://vitejs.dev/)
[![Vue.js](https://img.shields.io/badge/vuejs-%2335495e.svg?style=flat&logo=vuedotjs&logoColor=%234FC08D)](https://vuejs.org/)
[![TailwindCSS](https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=flat&logo=tailwind-css&logoColor=white)](https://tailwindcss.com/)
[![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=flat&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![DaisyUI](https://img.shields.io/badge/daisyui-5A0EF8?style=flat&logo=daisyui&logoColor=white)](https://daisyui.com/)

[Features](#-features) • [Getting Started](#-getting-started) • [Tech Stack](#-tech-stack) • [Releases](#-releases) • [Roadmap](#️-roadmap)

</div>

## ✨ Features

- 📝 Generate 20 random words for typing practice
- ⏱️ Real-time statistics tracking:
  - Elapsed time
  - Words per minute (WPM)
  - Accuracy percentage
- 🎯 Immediate visual feedback with color-coded characters
- ⌫ Character deletion support
- 👥 Contributors page
- 🎨 Sleek, minimalist black theme

## 🚀 Getting Started

> **Note**: This application is optimized for desktop use. Mobile support is not currently available.

```bash
# Clone the repository
git clone https://github.com/EmersonYang3/SwiftType

# CD to APP
cd APP

# Install dependencies
npm install

# Start development server
npm run dev
```

## 🛠️ Tech Stack
- **[Vue.js 3](https://vuejs.org/)** - Progressive JavaScript Framework
- **[Vite](https://vitejs.dev/)** - Next Generation Frontend Tooling
- **[Tailwind CSS](https://tailwindcss.com/)** - Utility-first CSS Framework
- **[TypeScript](https://www.typescriptlang.org/)** - JavaScript with Syntax for Types
- **[Daisy UI](https://daisyui.com/)** - Tailwind CSS Component Library

## 📦 Releases

### v1.0.0 - January 19, 2025
- Initial release with core typing test functionality
- Real-time statistics tracking
- Color-coded character feedback
- Basic contributor system

## 🗺️ Roadmap

Have a suggestion? Message Me:

- 💬 Discord: `0x5f1fff9`

## 👥 Contributors
- Lucille - _Project Creator_

## ☕ Special Thanks
Thanks for checking out SwiftType!

---